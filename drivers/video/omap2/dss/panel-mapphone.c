...
#include "../../../arch/arm/mach-omap2/dt_path.h"
#include <asm/prom.h>

#include <video/panel.h>

#define CONFIG_ARM_OF
#define CONFIG_USER_PANEL_DRIVER
...
{
	struct mapphone_data *map_data = dev_get_drvdata(&dssdev->dev);//(struct mapphone_data *) dssdev->data;
...
static void panel_print_dt(struct omap_dss_device *dssdev)
{
	...
	printk(KERN_DEBUG "DT: regn=%d regm=%d regm3=%d regm4=%d"
		...
		dssdev->clocks.dsi.lp_clk_div, dssdev->clocks.dispc.channel.lck_div,
				...
				struct device_attribute *attr, char *buf)
{
	...
	struct omap_dss_device *dssdev = to_dss_device(dev);
	dsi_bus_lock(dssdev);
	ret = sprintf(buf, "%d\n", mapphone_panel_read_supplier_id(dssdev));
	dsi_bus_unlock(dssdev);
	return ret;
}
...
static int dsi_mipi_280_vm_320_240_panel_enable(struct omap_dss_device *dssdev)
{
	...
	data[1] = 0x00;
	ret = dsi_vc_write(dssdev, dsi_vc_cmd, EDISCO_SHORT_WRITE_1, data, 2);
...
static int dsi_mipi_cm_480_854_panel_enable(struct omap_dss_device *dssdev)
{
	...
	if (dssdev->panel.panel_id == MOT_DISP_370_MIPI_480_854_CM) {
		printk(KERN_INFO "te_scan_line is set = %d \n",
				map_data->te_scan_line);
		if (mapphone_panel_read_supplier_id(dssdev) == SUPPLIER_ID_TMD) {
	...
	data[0] = EDISCO_CMD_SET_MCS;
	data[1] = 0x00;
	ret = mapphone_panel_lp_cmd_wrt_sync(dssdev, false, EDISCO_SHORT_WRITE_1,
						data, 2,
	...
	if (dssdev->panel.panel_id == MOT_DISP_MIPI_480_854_CM)
		...
		* 0x63 or 2-lanes, 0x60 for 1-lane (1st source displ only)*/
		ret = mapphone_panel_lp_cmd_wrt_sync(dssdev, true, 0x00,
					data, 4,
					0xef, 1,
					0x63, 0x63);
	else
	...
	ret = mapphone_panel_lp_cmd_wrt_sync(dssdev, true, 0,
			data, 1,
	...
	data[0] = EDISCO_CMD_EXIT_SLEEP_MODE;
	ret = mapphone_panel_lp_cmd_wrt_sync(dssdev, true, 0x0,
			data, 1,
					...
					struct omap_dss_device *dssdev)
{
	...
	ret = dsi_vc_set_max_rx_packet_size(dssdev, dsi_vc_cmd, 6);
	if (ret)
		printk(KERN_ERR "failed to set max_rx__packet_size\n");

	memset(data, 0, sizeof(data));

	ret = dsi_vc_dcs_read(dssdev, dsi_vc_cmd, EDISCO_CMD_READ_DDB_START,
					data, 6);
...
static bool mapphone_panel_deep_sleep_mode(struct omap_dss_device *dssdev)
{
	...
	if (ret == true) {
		if (dsi_vc_dcs_write_nosync(dssdev, dsi_vc_cmd, data, 2))
				      ...
				      u16 x, u16 y, u16 w, u16 h)
{
	...
	data[3] = (y + h - 1) >> 8;
	data[4] = (y + h - 1) & 0xff;
	ret = dsi_vc_dcs_write_nosync(dssdev, dsi_vc_cmd, data, 5);
	if (ret)
		return;
...
static int mapphone_panel_enable_te(struct omap_dss_device *dssdev, bool enable)
{
	...
	if (enable == true) {
		...
		data[1] = 0x00;
		ret = dsi_vc_dcs_write_nosync(dssdev, dsi_vc_cmd, data, 2);
		if (ret)
			goto error;
		...
		ret = dsi_vc_dcs_write_nosync(dssdev, dsi_vc_cmd, data, 3);
		if (ret)
			goto error;
	} else {
					...
					int test_num)
{
	...
	if (test_num == 1)	{
		if ((display->panel.panel_id == MOT_DISP_430_MIPI_480_854_CM) ||
			...
			dsi_vc_set_max_rx_packet_size(display, dsi_vc_cmd,
					SUPPLIER_ID_LEN);
			if (dsi_vc_dcs_read(display, dsi_vc_cmd,
			...
			dsi_vc_set_max_rx_packet_size(display, dsi_vc_cmd, 1);
		} else {
...
static struct omap_dss_driver mapphone_panel_driver = {
	...
	.setup_update2		= mapphone_panel_setup_update,
