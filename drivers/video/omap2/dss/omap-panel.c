...
static int omap_panel_set_fod(struct omap_panel_fod *arg)
{
	...
	if (en != entry->fod_en) {
		...
		if (!en && !entry->dss_en && entry->panel_en &&
			...
			dsi_bus_lock(entry->dev.dssdev);
			entry->panel_en = 0;
			entry->dev.fod_disable(entry->dev.dssdev);
			dsi_bus_unlock(entry->dev.dssdev);
		}
	}
...
static long omap_panel_ioctl(struct file *file, u_int cmd, u_long arg)
{
	int rc = 0;
...
static const struct file_operations omap_panel_fops = {
	...
	.unlocked_ioctl	= omap_panel_ioctl,
