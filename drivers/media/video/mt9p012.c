...
#define MT9P012_MODES_COUNT ARRAY_SIZE(sensor_settings)

static unsigned isize_current = MT9P012_MODES_COUNT - 1;
			    ...
			    u16 reg, u32 *val)
{
	...
	//*val = i2c_smbus_read_word_swapped(client, reg);
...
static int mt9p012_write_reg(struct i2c_client *client, u16 data_length,
			     u16 reg, u32 val)
{
	//i2c_smbus_write_word_swapped(client, reg, val);
	//return 0;
...
static int mt9p012_detect(struct i2c_client *client)
{
	...
	if (mt9p012_read_reg(client, MT9P012_16BIT, REG_MODEL_ID, &model_id)) {
		printk("read model_id failed\n");
		return -ENODEV;
	}
	if (mt9p012_read_reg(client, MT9P012_8BIT, REG_MANUFACTURER_ID,
				&mfr_id)) {
				printk("read manufacturer_id failed\n");
		return -ENODEV;
	}
	...
	{
		printk("write reset_data failed\n");
		return -ENODEV;
	}
	...
	if (mt9p012_read_reg(client, MT9P012_16BIT, REG_SENSOR_REVISION,
				&data))
	{
		printk("read sensor_revision failed\n");
		return -ENODEV;
	}
	...
	if (mt9p012_write_reg(client, MT9P012_16BIT, REG_RESET_REGISTER,
				reset_data))
	{
		printk("write reset_data2 failed\n");
		return -ENODEV;
	}
...
{
	struct mt9p012_frame_settings *frm;

	frm = &sensor_settings[isize_current].frame;
	pix->left = 0;
	pix->top = 0;
	pix->width =  frm->x_odd_inc;
	pix->height = frm->y_odd_inc;
